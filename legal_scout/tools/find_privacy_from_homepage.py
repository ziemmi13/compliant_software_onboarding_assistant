...
def _build_fallback_privacy_urls(base_url: str) -> list[str]:
    base = base_url if base_url.endswith("/") else f"{base_url}/"
    candidates: list[str] = []
    seen: set[str] = set()

    for token in COMMON_PRIVACY_PATHS:
...
def _extract_related_links(page_url: str, html: str) -> list[str]:
...
def find_privacy_from_homepage(base_url: str) -> dict[str, list[str]]:
    ...
    if not valid_links:
        for candidate_url in _build_fallback_privacy_urls(base_url):
    ...
    for page_url, html in candidate_pages:
        for related_link in _extract_related_links(page_url, html):
            response = _request(related_link, headers)
            if response is None:
                continue
            if response.status_code in BLOCKED_STATUS_CODES:
                add_blocked(related_link)
                continue
            if response.ok:
                add_valid(response.url)
